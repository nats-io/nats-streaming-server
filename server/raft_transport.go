...
func (n *natsStreamLayer) Dial(address raft.ServerAddress, timeout time.Duration) (net.Conn, error) {
	...
	connect := &connectRequestProto{
		...
		Inbox: fmt.Sprintf(natsRequestInbox, n.localAddr.String(), nats.NewInboxWithPath("_RAFT._DIAL")),
	}
...
func (n *natsStreamLayer) Accept() (net.Conn, error) {
	for {
		...
		inbox := fmt.Sprintf(natsRequestInbox, n.localAddr.String(), nats.NewInboxWithPath("_RAFT._PEERS."))
